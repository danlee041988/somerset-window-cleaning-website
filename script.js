...
if (contactForm) {
  contactForm.addEventListener('submit', async e => {
    ...
    if (formMessage) {
      formMessage.textContent = `Thank you for your enquiry, ${data.name}! We'll get back to you soon.`;
    }
    contactForm.reset();
  });
...
function updateReviewSummary() {
  ...
  if (serviceSummary) {
    const propertyTypeText =
      document.querySelector(`#propertyType option[value="${formState.propertyType}"]`)
        ?.textContent || '';
    const basePrice = propertyPrices[formState.propertyType] || 0;
    const adjustment = frequencyAdjustments[formState.frequency] || 0;
    const totalPrice = basePrice + adjustment;
  ...
  if (priceSummary) {
    ...
    if (basePrice > 0) {
      ...
      if (
        formState.additionalServices.includes('gutterInternal') &&
        formState.additionalServices.includes('gutterExternal')
      ) {
        priceHTML +=
          '<p class="special-offer">✨ Special offer: Windows cleaned FREE with both gutter services!</p>';
      }
  ...
  bookingForm.addEventListener('submit', async e => {
    ...
    const bookingData = {
      ...
      estimatedPrice: propertyPrices[formState.propertyType]
        ? propertyPrices[formState.propertyType] + (frequencyAdjustments[formState.frequency] || 0)
        : null,
    ...
    try {
      const response = await fetch(
        'https://window-cleaning-booking-system-6k15.vercel.app/api/submit-booking',
        {
          method: 'POST',
          headers: {
      ...
      if (result.success) {
        ...
        showSuccessMessage(
          `Thank you for your booking, ${formState.fullName}! Your booking reference is: ${result.bookingReference}. We'll contact you within 24 hours.`
        );
      ...
      if (error.message && error.message.includes('Valid UK mobile number required')) {
        errorMessage =
          'Please check your phone number format. We accept UK mobile and landline numbers';
      } else if (error.message && error.message.includes('Valid UK postcode required')) {
        errorMessage =
          'Please check your postcode format. UK postcodes should be in the format like BS1 4DJ or M1 1AA';
        ...
        errorMessage =
          'There was an error submitting your booking. Please try again or contact us directly at 01458 860339.';
      }
  ...
  if (recaptchaContainer && window.grecaptcha) {
    try {
      ...
      recaptchaContainer.innerHTML =
        '<p style="padding: 15px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px; color: #666;">reCAPTCHA verification required for form submission</p>';
    }
  }
}
